describe(`payment interceptors`, () => {
  ...
  describe(`a list of payments`, () => {
    beforeEach(() => {
      cy.intercept("POST", "/payment/insert", {fixture: "payment-insert.json"}).as("payment-insert")
      cy.intercept("GET", "/payment/select", { fixture: "payments.json" }).as(
        "payments"
      );

      cy.intercept("GET", "/account/select/active", { fixture: "accounts.json" }).as(
          "accounts"
      );
    ...
    it(`a message should be sent to describe the empty list`, () => {
      ...
      cy.get('.css-yk16xz-control > .css-g1d714-ValueContainer > .css-1wa3eu0-placeholder')
      ...
      cy.get('div > :nth-child(1) > .MuiIconButton-label > .material-icons').click();
      /* ==== End Cypress Studio ==== */
    });
  });
});
